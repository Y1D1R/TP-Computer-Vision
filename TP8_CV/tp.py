...
for key,val in faces.items():
    ...
    if key == "s11/1.pgm" or key == "s9/1.pgm":
        continue # this is our test set
...
query = faces["s11/1.pgm"].reshape(1,-1)
...
query = faces["s9/1.pgm"].reshape(1,-1)
query_weight = eigenfaces @ (query - pca.mean_).T
euclidean_distance = np.linalg.norm(weights - query_weight, axis=0)
best_match = np.argmin(euclidean_distance)
print("Best match %s with Euclidean distance %f" % (facelabel[best_match], euclidean_distance[best_match]))
...
title = "Random face with "+str(n_components)+" eigen vectors"
print(title)
axes[1].set_title(title)
plt.show()
"""
query = faces["s10/1.pgm"].reshape(1,-1)
query_weight = eigenfaces @ (query - pca.mean_).T
euclidean_distance = np.linalg.norm(weights - query_weight, axis=0)
best_match = np.argmin(euclidean_distance)
print("Best match %s with Euclidean distance %f" % (facelabel[best_match], euclidean_distance[best_match]))
...
query = faces["s10/1.pgm"].reshape(1,-1)
query_weight = (eigenfaces @ (query - pca.mean_).T) * weights.std()
print(query_weight.shape)
query_weight = query_weight[:,0]
print(query_weight.shape)
fig, axes = plt.subplots(1,2,sharex=True,sharey=True,figsize=(8,6))
newface = query_weight @ eigenfaces + pca.mean_
axes[1].imshow(newface.reshape(faceshape), cmap="gray")
title = "new face with "+str(n_components)+" eigen vectors"
print(title)
axes[1].set_title(title)
plt.show()
